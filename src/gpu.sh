...
if [ -z "$GPU_INFO" ]; then
    ...
    yay -Sy --needed --noconfirm cuda-toolkit egl-wayland mesa mesa-utils lib32-mesa-minimal vulkan-tools
fi
if echo "$GPU_INFO" | grep -qi "NVIDIA"; then
    ...
    if echo "$GPU_INFO" | grep -qi "NVIDIA Corporation TU"; then
        ...
        yay -Sy --needed --noconfirm nvidia
    else
...
if echo "$GPU_INFO" | grep -qi "Intel"; then
    ...
    yay -Sy --needed --noconfirm intel-media-driver vulkan-intel lib32-vulkan-intel libva-intel-driver libva-utils
fi
fi
if echo "$GPU_INFO" | grep -qi "AMD"; then
    GPU_TYPE="amd"
    echo "installing amd drivers"
    yay -Sy --needed --noconfirm xf86-video-amdgpu libva-mesa-driver lib32-libva-mesa-driver
fi
